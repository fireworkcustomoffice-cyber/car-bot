...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Помогу разобраться в пригоне авто из-за рубежа, прицениться и ответить на вопросы.\n\n"
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Оставь свой номер телефона или нажми «Пропустить».\n"
        "Это нужно, чтобы менеджер мог связаться с тобой, если понадобится.",
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.contact:
        ...
        user_data[user_id]["phone"] = entered_text
        logging.info(f"Новый клиент: {name}, контакт: {entered_text}, tg_id: {user_id}")

    await update.message.reply_text(
        f"Отлично, {name}. Теперь я готов помочь.\n\n"
        "Спрашивай всё, что интересует: из каких стран везём, сколько стоит, какие сроки, как работает растаможка.\n"
        "Если понадобится точный расчёт — передам тебя менеджеру Евгению.",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if name:
            system_prompt += f"\n\nКлиента зовут {name}. Иногда обращайся к нему по имени."

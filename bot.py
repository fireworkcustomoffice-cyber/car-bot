...
CARS_DATABASE: List[Dict[str, Any]] = [
    {
        ...
        "power_hp": 155, "engine_cc": 1500, "fuel": "бензин",
        "price_usd": 18000, "is_under_160_hp": True, "is_available": True, "priority": True,
        "url": "https://che168.com", "comment": "Проходной кроссовер до 160 л.с.",
    },
    {
        "brand": "Volkswagen", "model": "Tiguan L", "year": 2024, "country": "Китай",
        "power_hp": 160, "engine_cc": 1400, "fuel": "бензин",
        "price_usd": 20000, "is_under_160_hp": True, "is_available": True, "priority": True,
        "url": "https://che168.com", "comment": "Семейный кроссовер с хорошей ликвидностью.",
    },
    {
        "brand": "Toyota", "model": "Camry", "year": 2024, "country": "Китай",
        "power_hp": 173, "engine_cc": 2000, "fuel": "бензин",
        "price_usd": 22000, "is_under_160_hp": False, "is_available": True, "priority": False,
        "url": "https://che168.com", "comment": "Популярная модель, выше 160 л.с.",
    },
    {
        "brand": "Haval", "model": "H6", "year": 2025, "country": "Китай",
        "power_hp": 150, "engine_cc": 1500, "fuel": "бензин",
        "price_usd": 15000, "is_under_160_hp": True, "is_available": True, "priority": True,
        "url": "https://che168.com", "comment": "Бюджетный кроссовер с хорошим оснащением.",
    },
    {
        "brand": "Li Auto", "model": "L6", "year": 2024, "country": "Китай",
        "power_hp": 330, "engine_cc": 1500, "fuel": "гибрид",
        "price_usd": 32000, "is_under_160_hp": False, "is_available": True, "priority": False,
        "url": "https://che168.com", "comment": "Премиальный гибрид.",
    },
]

# Словарь популярных авто для расчёта (включая те что не в базе)
# Ключи — все варианты написания (рус/англ/сокращения)
KNOWN_CARS_DICT: Dict[str, Dict] = {
    # Audi
    ...
    "тойота хайлендер": {"brand": "Toyota", "model": "Highlander", "engine_cc": 2500, "power_hp": 238, "price_usd": 40000},
    # Lexus
    ...
    "li auto": {"brand": "Li Auto", "model": "L6", "engine_cc": 1500, "power_hp": 330, "price_usd": 32000},
    # Porsche
    "porsche cayenne": {"brand": "Porsche", "model": "Cayenne", "engine_cc": 3000, "power_hp": 340, "price_usd": 85000},
    "порш кайен": {"brand": "Porsche", "model": "Cayenne", "engine_cc": 3000, "power_hp": 340, "price_usd": 85000},
...
Тебя зовут Игорь. Ты настоящий ИИ-консультант.

Тон: уверенный, спокойный, дружелюбный. Без воды. Живой язык.

Правила:
1. ВСЕГДА слушай запрос — если клиент назвал марку/модель, работай именно с ней
...
8. Если спросят кто ты — честно скажи что ИИ-ассистент

Когда показываешь расчёт — разбивай по статьям, показывай вилку от/до.
...
async def get_usd_rate() -> float:
    ...
    try:
        ...
        logging.error(f"Курс ЦБ недоступен: {e}")
        return 90.0


def calc_customs_duty(price_eur: float, engine_cc: int, age: int) -> float:
    if age < 3:
        if price_eur <= 8500:    return max(price_eur * 0.54, engine_cc * 2.5)
        elif price_eur <= 16700: return max(price_eur * 0.48, engine_cc * 3.5)
        elif price_eur <= 42300: return max(price_eur * 0.48, engine_cc * 5.5)
        elif price_eur <= 84500: return max(price_eur * 0.48, engine_cc * 7.5)
        elif price_eur <= 169000:return max(price_eur * 0.48, engine_cc * 15.0)
        else:                    return max(price_eur * 0.48, engine_cc * 20.0)
    elif age <= 5:
        if engine_cc <= 1000:   return engine_cc * 1.5
        elif engine_cc <= 1500: return engine_cc * 1.7
        elif engine_cc <= 1800: return engine_cc * 2.5
        elif engine_cc <= 2300: return engine_cc * 2.7
        elif engine_cc <= 3000: return engine_cc * 3.0
        else:                   return engine_cc * 3.6
    else:
        if engine_cc <= 1000:   return engine_cc * 3.0
        elif engine_cc <= 1500: return engine_cc * 3.2
        elif engine_cc <= 1800: return engine_cc * 3.5
        elif engine_cc <= 2300: return engine_cc * 4.8
        elif engine_cc <= 3000: return engine_cc * 5.0
        else:                   return engine_cc * 5.7

def calc_excise(power_hp: int) -> float:
    if power_hp <= 90:   return 0
    elif power_hp <= 150: return power_hp * 55
    elif power_hp <= 200: return power_hp * 531
    elif power_hp <= 300: return power_hp * 869
    else:                 return power_hp * 1063

def calc_util(engine_cc: int, age: int) -> float:
    return 20000 * (0.17 if age < 3 else 0.26)

async def do_calc(price_usd: float, engine_cc: int, power_hp: int,
                  year: int, usd_rate: float) -> Dict:
    rate = usd_rate + 2
    eur_rate = rate * 0.93
    age = 2025 - year
    ...
    duty = calc_customs_duty(price_usd * 0.93, engine_cc, age) * eur_rate
    util = calc_util(engine_cc, age)
    excise = calc_excise(power_hp)
...
def format_calc(label: str, c: Dict) -> str:
    ...
    return (
        ...
        f"📦 Оформление в Китае/стране: {r(c['china'])} ₽\n"
...
def find_car_by_name(text: str) -> Optional[Dict]:
    """Ищем авто сначала в базе, потом в словаре популярных"""
    t = normalize(text)
    # Сначала база
    best_db = None
    best_db_score = 0
    ...
    best_key = None
    best_len = 0
    for key in KNOWN_CARS_DICT:
        k = key.strip()
        if k in t and len(k) > best_len:
            best_key = key
            best_len = len(k)
    if best_key:
        return {"source": "dict", "data": KNOWN_CARS_DICT[best_key]}

    return None

def search_cars_for_context(query: str, budget_rub: Optional[int],
                             countries: List[str], under_160: bool,
                             usd_rate: float) -> List[Dict]:
    q = normalize(query)
    results = []
    for car in CARS_DATABASE:
        ...
        if budget_rub and car["price_usd"] * (usd_rate + 2) * 1.5 > budget_rub: continue
        score = 0
...
def build_context(user_id: int, text: str, usd_rate: float) -> str:
    ...
    cars = search_cars_for_context(text, budget, countries, under_160, usd_rate)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wants_calc(llm_input):
        ...
        found = find_car_by_name(llm_input)
        ...
        if found:
            if found["source"] == "db":
                ...
                year = year_override or car["year"]
            ...
            calc = await do_calc(price_usd, engine_cc, power_hp, year, usd_rate)
            ...
            await update.message.reply_text(
                ...
                "Например: «посчитай BMW X5 2022» или «Camry 2023 под ключ».\n\n"
                "Или свяжись с менеджером для индивидуального расчёта: @superluxxx",
                reply_markup=main_keyboard()
            )
            return CHAT
